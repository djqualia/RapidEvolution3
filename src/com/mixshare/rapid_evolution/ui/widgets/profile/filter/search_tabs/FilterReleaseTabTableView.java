...
public class FilterReleaseTabTableView extends FilterTabTableView {
	
	static private Logger log = Logger.getLogger(FilterReleaseTabTableView.class);
	
	private QAction removeAction;
	
	public FilterReleaseTabTableView(RecordTableModelManager modelManager) {
		super(modelManager);
		
		removeAction = new QAction(Translations.get("remove_text"), this);
		removeAction.triggered.connect(this, "removeReleases()");
	...
	public void dropEvent(QDropEvent event) {				
		...
		filter.getFilterRecord().addReleaseRecords(releases);
		filter.getFilterRecord().computeNumReleaseRecords();
	...
	private void removeReleases() {
		...
		Vector<SearchRecord> selectedRecords = getSelectedRecords();
		Vector<ReleaseRecord> selectedReleases = new Vector<ReleaseRecord>(selectedRecords.size());
		FilterRecord filterRecord = ((FilterProfile)ProfileWidgetUI.instance.getCurrentProfile()).getFilterRecord();
		if (log.isDebugEnabled())
			log.debug("removeReleases(): removing releases=" + selectedReleases + ", from filter=" + filterRecord);			
		for (SearchRecord record : selectedRecords)
			selectedReleases.add((ReleaseRecord)record);
		filterRecord.removeReleaseRecords(selectedReleases);
		boolean containsInternal = false;
		boolean containsExternal = false;
		for (ReleaseRecord release : selectedReleases) {
		...
		if (containsExternal)
			filterRecord.computeNumExternalReleaseRecords();			
		filterRecord.update();
	}	
