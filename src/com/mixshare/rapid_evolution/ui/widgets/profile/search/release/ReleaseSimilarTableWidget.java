...
import com.mixshare.rapid_evolution.ui.model.profile.search.release.SimilarReleaseProxyModel;
...
public class ReleaseSimilarTableWidget extends RecordTabTableWidget {

	private SimilarTabTableProxyModel similarProxy;
	
	public ReleaseSimilarTableWidget(RecordTableModelManager modelManager, Column defaultSortColumn) {
	...
	public RecordTabTableProxyModel getTabTableProxyModel(RecordTabTableWidget tabTableWidget, RecordTableModelManager tableManager, RecordTabTableView tabTableView) {
		similarProxy = new SimilarReleaseProxyModel(tabTableWidget, tableManager, tabTableView);
		if (UIProperties.hasProperty("release_similarity_show_type"))
			similarProxy.setFilterType(UIProperties.getByte("release_similarity_show_type"));		
		return similarProxy;
	}
	...
	public byte getShowType() {
		if (UIProperties.hasProperty("release_similarity_show_type"))
			return UIProperties.getByte("release_similarity_show_type");
		return 0;
	}
	...
	public void setShowType() {
    	if (similarProxy != null) {
    		...
    		UIProperties.setProperty("release_similarity_show_type", String.valueOf(searchBarWidget.getFilterCombo().currentIndex()));
